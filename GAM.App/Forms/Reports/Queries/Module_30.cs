    ...
    class Module_30
    {
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            for (int i = 1; i <= dates.Length; i++)
                branchesTable.Columns.Add(new DataColumn { ColumnName = "PrimaryColumns.Mandeh" + i, Caption = UDF.GetDateString(dates[i - 1]), DataType = Type.GetType("System.Double"), Namespace = UDF.GetDateString(dates[i - 1]) });
            ...
            foreach (Branches.BranchInfo bi in Branches.GetAllBranchs())
            {
                if (groupByBranchId.ContainsKey(bi.BranchId))
                {
                    ...
                    for (int i = 1; i <= dates.Length; i++)
                    {
                        double mandeh = Numeral.AnyToDouble(row.Values.Where(x => x["ReportDate"].ToString() == dates[i - 1]).First()["Mandeh"]);
                        newRow["PrimaryColumns.Mandeh" + i] = mandeh;

                        seriesPoints.Add(new ISeries.DataPoint(bi.BranchId, dates[i - 1], "عملکرد", mandeh));
                    }
                ...
                else
                {
                    ...
                    newRow["Branches.BranchDegree"] = bi.BranchDegree;
                    for (int i = 1; i <= dates.Length; i++)
                        newRow["PrimaryColumns.Mandeh" + i] = 0;
            ...
            for (int i = 1; i <= dates.Length; i++)
                domainsTable.Columns.Add(new DataColumn { ColumnName = "PrimaryColumns.Mandeh" + i, Caption = UDF.GetDateString(dates[i - 1]), DataType = Type.GetType("System.Double"), Namespace = UDF.GetDateString(dates[i - 1]) });
            ...
            foreach (Branches.DomainInfo di in Branches.GetDomains())
            {
                if (groupByBranchId.ContainsKey(di.DomainId))
                {
                    ...
                    for (int i = 1; i <= dates.Length; i++)
                    {
                        ...
                        seriesPoints.Add(new ISeries.DataPoint(di.DomainId, dates[i - 1], "عملکرد", mandeh));
                    }
            ...
            for (int i = 1; i <= dates.Length; i++)
                provinceTable.Columns.Add(new DataColumn { ColumnName = "PrimaryColumns.Mandeh" + i, Caption = UDF.GetDateString(dates[i - 1]), DataType = Type.GetType("System.Double"), Namespace = UDF.GetDateString(dates[i - 1]) });
            ...
            foreach (Branches.ProvinceInfo pi in Branches.GetProvincesByZoneId(Users.MyZoneID))
            {
                ...
                if (groupByBranchId.ContainsKey(pi.ProvinceId))
                {
                    ...
                    for (int i = 1; i <= dates.Length; i++)
                    {
                        ...
                        seriesPoints.Add(new ISeries.DataPoint(pi.ProvinceId, dates[i - 1], "عملکرد", mandeh));
                    }
