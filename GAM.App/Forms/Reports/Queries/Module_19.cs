...
namespace GAM.Forms.Reports.Queries
{
    class Module_19
    {
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            var categories = Categories.BySection().OrderBy(x => x.CategoryId).Select(x => new { x.CategoryId, x.CategoryName }).Distinct();
            ...
            var SelectedCustomers = tblMaster.AsEnumerable().OrderByDescending(x => Numeral.AnyToDouble(x["ContractPayCost"]))
                               .Select(r => new
                               {
                                   ...
                                   Category = Categories.GetCategory(Categories.BySection(), r["EconomicSector"].ToString(), r["TypeOfFacility"].ToString()),
