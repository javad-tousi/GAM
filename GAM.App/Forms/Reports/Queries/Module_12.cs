...
namespace GAM.Forms.Reports.Queries
{
    class Module_12
    {
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            customersTable.Columns.Add(new DataColumn { ColumnName = "Customers.FacilityName", Caption = "نوع تسهیلات", DataType = Type.GetType("System.String") });
            ...
            foreach (var g in SelectedCustomers)
            {
                ...
                newRow["Customers.FacilityName"] = g.FacilityName;
                ...
                if (groupByBranchId.ContainsKey(branchId))
                {
                    ...
                    foreach (var item in categoriesList)
                    {
                        if (item.Key > 0)
                        {
                            ...
                            newRow[item.Value + ".Total"] = row.Values.Where(x => int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                        else 
                        {
            ...
            customersTable.Columns["Categories.CategoryId"].ExtendedProperties["Visible"] = false;
