...
namespace GAM.Forms.Reports.Queries
{
    class Module_13
    {
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            var categories = Categories.ByAmount1().OrderBy(x => x.CategoryId).Select(x => new { x.CategoryId, x.CategoryName }).Distinct();
            IDictionary<int, string> categoriesList1 = new Dictionary<int, string>();
            IDictionary<int, string> categoriesList2 = new Dictionary<int, string>();
            IDictionary<int, string> categoriesList3 = new Dictionary<int, string>();
            ...
            bands.Add(gvm.CreateBand("Sarresid", "سررسیدگذشته"));
            bands.Add(gvm.CreateBand("Moavagh", "معوق"));
            bands.Add(gvm.CreateBand("Mashkook", "مشکوک الوصول"));
            bands.Add(gvm.CreateBand("Debt", "مانده در مطالبات به تاریخ " + UDF.GetDateString(dates[0])));
            bands.Add(gvm.CreateBand("Sum", "جمع"));
            ...
            {
                ++catNo;
                categoriesList1.Add(cat.CategoryId, "Sarresid.Cat" + catNo);
            ...
            {
                ++catNo;
                categoriesList2.Add(cat.CategoryId, "Moavagh.Cat" + catNo);
            }
            ...
            {
                ++catNo;
                categoriesList3.Add(cat.CategoryId, "Mashkook.Cat" + catNo);
            }
            categoriesList1.Add(0, "Sarresid.Sum");
            categoriesList2.Add(0, "Moavagh.Sum");
            categoriesList3.Add(0, "Mashkook.Sum");
            #endregion
            ...
            foreach (var item in categoriesList1)
            {
            ...
            foreach (var item in categoriesList2)
            {
                branchesTable.Columns.Add(new DataColumn { ColumnName = item.Value + ".Count", Caption = "تعداد", DataType = Type.GetType("System.Int32") });
                branchesTable.Columns.Add(new DataColumn { ColumnName = item.Value + ".Total", Caption = "مبلغ", DataType = Type.GetType("System.Double") });
            }

            foreach (var item in categoriesList3)
            {
                branchesTable.Columns.Add(new DataColumn { ColumnName = item.Value + ".Count", Caption = "تعداد", DataType = Type.GetType("System.Int32") });
                branchesTable.Columns.Add(new DataColumn { ColumnName = item.Value + ".Total", Caption = "مبلغ", DataType = Type.GetType("System.Double") });
            }
            ...
            var groupByBranchId = customersTable.AsEnumerable().GroupBy(dr => new { BranchId = dr["Branches.BranchId"].ToString() })
                                   .Select(g => new
                                   {
                                       ...
                                       Values = g,
                                       SubCount = g.Count(),
                                       SubTotal = g.Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"])),
            ...
            foreach (Branches.BranchInfo bi in Branches.GetAllBranchs())
            {
                ...
                if (groupByBranchId.ContainsKey(branchId))
                {
                    ...
                    foreach (var item in categoriesList1)
                    {
                        if (item.Key > 0)
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "سر رسید گذشته" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "سر رسید گذشته" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                        else
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "سر رسید گذشته").Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "سر رسید گذشته").Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                    }
                    foreach (var item in categoriesList2)
                    {
                        if (item.Key > 0)
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "معوق" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "معوق" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                        else
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "معوق").Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "معوق").Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                    }
                    foreach (var item in categoriesList3)
                    {
                        if (item.Key > 0)
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "مشکوک الوصول" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "مشکوک الوصول" & int.Parse(x["Categories.CategoryId"].ToString()) == item.Key).Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                        else
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "مشکوک الوصول").Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => x["Customers.ContractStatus"].ToString() == "مشکوک الوصول").Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                    }
