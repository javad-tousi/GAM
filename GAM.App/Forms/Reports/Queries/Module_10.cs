        ...
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            var SelectedCustomers = tblMaster.AsEnumerable().OrderByDescending(x => Numeral.AnyToDouble(x["MandehGheyreJari"]))
                               .Select(r => new
                               {
                                   ...
                                   Category = Categories.GetCategory(Categories.BySection(), r["EconomicSector"].ToString(), r["FacilityName"].ToString())
                               });
            ...
            foreach (Branches.BranchInfo bi in Branches.GetAllBranchs())
            {
                ...
                if (groupByBranchId.ContainsKey(branchId))
                {
                    ...
                    foreach (var cat in categorieslist)
                    {
                        if (cat.Key > 0)
                        {
                            newRow[cat.Value + ".Count"] = row.Values.Where(x => int.Parse(x["Facility.CategoryId"].ToString()) == cat.Key).Count();
                            newRow[cat.Value + ".Total"] = row.Values.Where(x => int.Parse(x["Facility.CategoryId"].ToString()) == cat.Key).Sum(x => Numeral.AnyToDouble(x["Debt.MandehGheyreJari"]));
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    foreach (var cat in categorieslist)
                    {
                        newRow[cat.Value + ".Count"] = 0;
                        newRow[cat.Value + ".Total"] = 0;
                    }
                    branchesTable.Rows.Add(newRow);
                }
            }
            ...
            foreach (var cat in categorieslist)
            {
                domainsTable.Columns.Add(new DataColumn { ColumnName = cat.Value + ".Count", Caption = "تعداد", DataType = Type.GetType("System.Int32") });
                domainsTable.Columns.Add(new DataColumn { ColumnName = cat.Value + ".Total", Caption = "مبلغ", DataType = Type.GetType("System.Double") });
            }
            ...
            foreach (Branches.DomainInfo di in Branches.GetDomains())
            {
                ...
                if (brRows != null)
                {
                    ...
                    foreach (var cat in categorieslist)
                    {
                        newRow[cat.Value + ".Count"] = brRows.Sum(x => Numeral.AnyToDouble(x[cat.Value + ".Count"]));
            ...
            foreach (var cat in categorieslist)
            {
                provinceTable.Columns.Add(new DataColumn { ColumnName = cat.Value + ".Count", Caption = "تعداد", DataType = Type.GetType("System.Int32") });
                provinceTable.Columns.Add(new DataColumn { ColumnName = cat.Value + ".Total", Caption = "مبلغ", DataType = Type.GetType("System.Double") });
            }
            ...
            foreach (Branches.ProvinceInfo pi in Branches.GetProvincesByZoneId(Users.MyZoneID))
            {
                ...
                if (doRows != null)
                {
                    ...
                    foreach (var cat in categorieslist)
                    {
                        newRow[cat.Value + ".Count"] = doRows.Sum(x => Numeral.AnyToDouble(x[cat.Value + ".Count"]));
                        newRow[cat.Value + ".Total"] = doRows.Sum(x => Numeral.AnyToDouble(x[cat.Value + ".Total"]));
                    }
