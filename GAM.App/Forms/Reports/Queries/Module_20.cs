...
namespace GAM.Forms.Reports.Queries
{
    class Module_20
    {
        public static DataSet Fill(DataTable tblMaster, string[] dates, QueryProperties properties)
        {
            ...
            var categories = Categories.ByFacilityName().OrderBy(x => x.CategoryId).Select(x => new { x.CategoryId, x.CategoryName }).Distinct();
            ...
            bands.Add(gvm.CreateBand("Customers", "اطلاعات قرارداد"));
            bands.Add(gvm.CreateBand("Facility", "تسهیلات"));
            bands.Add(gvm.CreateBand("Pay", "حجم پرداختی طی دوره"));
            int catNo = 2;
            ...
            customersTable.Columns.Add(new DataColumn { ColumnName = "Customers.CustomerName", Caption = "نام مشتری", DataType = Type.GetType("System.String") });
            ...
            customersTable.Columns.Add(new DataColumn { ColumnName = "Facility.CategoryId", Caption = "کد", DataType = Type.GetType("System.Int32") });
            customersTable.Columns.Add(new DataColumn { ColumnName = "Facility.CategoryName", Caption = "دسته بندی", DataType = Type.GetType("System.String") });
            customersTable.Columns.Add(new DataColumn { ColumnName = "Facility.Name", Caption = "نوع تسهیلات", DataType = Type.GetType("System.String") });
            customersTable.Columns.Add(new DataColumn { ColumnName = "Pay.ContractRegDate", Caption = "تاریخ اعطاء", DataType = Type.GetType("System.String") });
            customersTable.Columns.Add(new DataColumn { ColumnName = "Pay.ContractPayCost", Caption = "مبلغ اعطایی", DataType = Type.GetType("System.Double") });

            var SelectedCustomers = tblMaster.AsEnumerable().OrderByDescending(x => Numeral.AnyToDouble(x["ContractPayCost"]))
                               .Select(r => new
                               {
                                   ...
                                   ContractRegDate = r["ContractRegDate"],
                                   ContractPayCost = Numeral.AnyToDouble(r["ContractPayCost"]),
                                   Category = Categories.GetCategory(Categories.ByFacilityName(), r["EconomicSector"].ToString(), r["TypeOfFacility"].ToString()),
                                   FacilityName = r["TypeOfFacility"].ToString()
                               });
            ...
            foreach (var g in SelectedCustomers)
            {
                ...
                newRow["Facility.CategoryId"] = g.Category.CategoryId;
                newRow["Facility.CategoryName"] = g.Category.CategoryName;
                newRow["Facility.Name"] = g.FacilityName;
                newRow["Pay.ContractRegDate"] = g.ContractRegDate;
                newRow["Pay.ContractPayCost"] = g.ContractPayCost;
            ...
            var groupByBranchId = customersTable.AsEnumerable().GroupBy(dr => new { BranchId = dr["Branches.BranchId"].ToString() })
                                   .Select(g => new
                                   {
                                       ...
                                       Values = g,
                                       SubCount = g.Count(),
                                       SubTotal = g.Sum(x => Numeral.AnyToDouble(x["Pay.ContractPayCost"])),
            ...
            foreach (Branches.BranchInfo bi in Branches.GetAllBranchs())
            {
                ...
                if (groupByBranchId.ContainsKey(branchId))
                {
                    ...
                    foreach (var item in categoriesList)
                    {
                        if (item.Key > 0)
                        {
                            newRow[item.Value + ".Count"] = row.Values.Where(x => int.Parse(x["Facility.CategoryId"].ToString()) == item.Key).Count();
                            newRow[item.Value + ".Total"] = row.Values.Where(x => int.Parse(x["Facility.CategoryId"].ToString()) == item.Key).Sum(x => Numeral.AnyToDouble(x["Pay.ContractPayCost"]));
                        }
                        else
                        {
            ...
            customersTable.Columns["Facility.CategoryId"].ExtendedProperties["Visible"] = false;
            customersTable.Columns["Pay.ContractPayCost"].ExtendedProperties.Add("FormatNumber", "N0");
            ...
            customersListTable.Columns["Pay.ContractPayCost"].ExtendedProperties.Add("FormatNumber", "N0");
